...
import { DeleteSong, GetSongByArtist, ISong } from "@/components/song-module/controller"
...
export function SongsTablesforArtist() {
 ...
 const loadSongs = async () => {
  ...
  console.log("The Artist Id",artistId)
  ...
  try {
    const res = await GetSongByArtist(token, artistId)
    console.log("GetSongByArtist response:", res.data);

    if (res.status === 200) {
      setSongs(res.data || []);
    } else {
      toast.error(res.message || "Failed to load songs");
    }
  } catch (error: any) {
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
        ...
        <Button
          onClick={() =>
            router.push("/dashboard-artist/artistSong/song-add")
          }
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              router.push(
                                `/dashboard-artist/artistSong/song-edit/${song._id}`
                              )
                            }
                          >
                            Edit
                            ...
                            onClick={() =>
                              router.push(
                                `/dashboard-artist/artistSong/artistSongView/${song._id}`
                              )
                            }
                          >
                            View
